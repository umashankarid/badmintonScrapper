...
def test_live_ensure_tournament():
    ...
    try:
        ...
        print(f"  ❌ Error: {e}")
        pytest.fail(f"Error cleaning old tournament record: {e}")
    ...
    try:
        ...
        if not result.get('success'):
            ...
            pytest.fail(f"Endpoint failed: {result.get('error')}")
        ...
        pytest.fail(f"Error calling endpoint: {e}. Make sure the Flask app is running on {BASE} (python3 app.py)")
    ...
    if not row:
        ...
        pytest.fail("No record found in tournaments.db after ensure_tournament() call")
