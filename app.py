        ...
        if "SJT" in tournament_name.upper():
            ...
            if k_row:
                ...
                if is_level_3_to_5 and not is_level_6:
                    ...
                    # LEVEL_6: can play both MJT and SJT — no restriction on MJT
                    # Only check if playing above their age group (needs permission)
                    import re as _re
                    conn_p = sqlite3.connect(PLAYERS_DB)
                    cur_p = conn_p.cursor()
                    cur_p.execute("SELECT dob FROM players WHERE license_id = ?", (license_id,))
...
def _send_admin_reg_closed_notification(tournament_name, admin_reg_end_date):
    ...
    try:
        ...
        body = (f"Hi Admin,\n\n"
                ...
                f"Se anmälningar: https://tournament-registration.bmkkomet.se\n\n\n"
                ...
                f"View registrations: https://tournament-registration.bmkkomet.se\n\n\n"
            ...
            for p_row in cur_p.fetchall():
                ...
                if days_left == 3:
                    ...
                    body = (f"Hi {player_name},\n\n"
                            ...
                            f"Don't want reminders? Log in and click 'Disable reminders'.\n\n"
                            f"Tävlingsfrågor / Tournament questions: tavlingar@bmkkomet.se\n"
                            f"Support / Hjälp: support@bmkkomet.se\n\n"
                            f"Med vänliga hälsningar / Best regards,\nBMK Komet")
                else:
...
def test_auto_reminders():
    ...
    subject2 = f"🏸 Anmälan stängd / Registration closed: {tournament_name}"
    body2 = (f"Hi Admin,\n\n"
             f"Anmälningsdeadlinen för '{tournament_name}' har passerats ({admin_reg_end_date}).\n"
             f"Det är dags att skicka in registreringarna till Svenska Badmintonförbundets hemsida.\n\n"
             f"📊 Totalt antal anmälningar: {reg_count} spelare\n\n"
    ...
    subject4 = f"⚠️ Sista dag att avanmäla / Last day to withdraw: {tournament_name}"
    body4 = (f"Hi {player_name},\n\n"
             f"Imorgon ({cancellation_deadline}) är sista dagen att avanmäla dig från '{tournament_name}' utan kostnad.\n\n"
             f"• Om du vill dra dig ur — avanmäl dig senast imorgon.\n"
             f"• Efter detta datum debiteras startavgiften oavsett om du deltar eller inte.\n\n\n"
             f"Tomorrow ({cancellation_deadline}) is the last day to withdraw from '{tournament_name}' free of charge.\n\n"
